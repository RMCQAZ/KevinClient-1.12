...
import kevin.module.modules.movement.ElytraFly;
import kevin.module.modules.movement.NoPush;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    public boolean isPotionActive(Potion potionIn)
    {
        ...
        if ((potionIn == MobEffects.NAUSEA || potionIn == MobEffects.BLINDNESS) && java.util.Objects.requireNonNull(antiBlind).getState() && antiBlind.getConfusionEffect().get())
            return false;
            ...
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
            {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    if (this.isElytraFlying())
                    {
                        ...
                        final double motX = this.motionX;
                        final double motY = this.motionY;
                        final double motZ = this.motionZ;
                        ...
                        if (KevinClient.moduleManager.getModule(ElytraFly.class).getState()) {
                            this.motionX = motX;
                            this.motionY = motY;
                            this.motionZ = motZ;
                        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (java.util.Objects.requireNonNull(KevinClient.moduleManager.getModule("AirJump")).getState() && isJumping && this.jumpTicks == 0) {
            this.jump();
            this.jumpTicks = 10;
        }
    ...
    {
        if (KevinClient.moduleManager.getModule(NoPush.class).getState()) return false;
