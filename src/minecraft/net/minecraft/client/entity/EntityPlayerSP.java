...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    private void onUpdateWalkingPlayer()
    {
        ...
        boolean flag = this.isSprinting() && !fakeSprint;
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            double d3 = (double)(yaw - lastReportedYaw);
            double d4 = (double)(pitch - lastReportedPitch);
    ...
    public void swingArm(EnumHand hand)
    {
        ...
        if (noSwing.getState()) {
            if (!noSwing.getServerSideValue().get()) this.connection.sendPacket(new CPacketAnimation(hand));
            return;
        }
        super.swingArm(hand);
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.inPortal)
        {
            if (this.mc.currentScreen != null && !this.mc.currentScreen.doesGuiPauseGame() && !KevinClient.moduleManager.getModule("PortalMenu").getState())
            {
                this.mc.displayGuiScreen((GuiScreen)null);
            }
        ...
        if (this.isSprinting() && ((!(sprint.getState() && sprint.getAllDirectionsValue().get()) && this.movementInput.moveForward < 0.8F) || this.isCollidedHorizontally || !flag4))
        {
            this.setSprinting(false);
        }
    ...
    public void move(MoverType type, double x, double y, double z)
    {
        switch (type) {
            case SELF: {
                ...
                PlayerMoveEvent event = PlayerMoveEvent.PlayerMoveEventPre(player);
                EventManager.INSTANCE.callEvent(event);
                ...
                if (event.isModified()) {
                    ...
                    EventManager.INSTANCE.callEvent(PlayerMoveEvent.PlayerMoveEventPost());
                    return;
                }
            }
            case PLAYER: {
