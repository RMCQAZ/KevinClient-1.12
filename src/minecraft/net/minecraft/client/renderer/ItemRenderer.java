...
public class ItemRenderer
{
    ...
    private void renderFireInFirstPerson()
    {
        ...
        float alpha = antiBlind.getState() ? antiBlind.getFireEffect().get() : 0.9F;
        GlStateManager.color(1.0F, 1.0F, 1.0F, alpha);
        GlStateManager.depthFunc(519);
        GlStateManager.depthMask(false);
        GlStateManager.enableBlend();
        GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
        float f = antiBlind.getState() && antiBlind.getFireEffect().get()!=0.9F ? 0.7F : 1.0F;
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
            bufferbuilder.pos(f5, f7, f9).tex((double)f2, (double)f4).endVertex();
            bufferbuilder.pos(f6, f7, f9).tex((double)f1, (double)f4).endVertex();
            bufferbuilder.pos(f6, f8, f9).tex((double)f1, (double)f3).endVertex();
            bufferbuilder.pos(f5, f8, f9).tex((double)f2, (double)f3).endVertex();
            tessellator.draw();
            GlStateManager.popMatrix();
        }
