...
import kevin.module.modules.render.ShulkerBoxPreview;
...
import static kevin.module.modules.render.ShulkerBoxPreview.shulkerBoxItems;
...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    protected void renderToolTip(ItemStack stack, int x, int y)
    {
        ...
        final ShulkerBoxPreview shulkerBoxPreview = (ShulkerBoxPreview) KevinClient.moduleManager.getModule(ShulkerBoxPreview.class);
        if (shulkerBoxPreview.getState() && shulkerBoxPreview.getGuiValue().get() && !stack.isEmpty() && stack.getItem() instanceof ItemShulkerBox) {
            ...
            var width = 18 * 9 / 2;
            var height = 18 * 3 / 2;
            var scale = shulkerBoxPreview.getGuiScaleValue().get();
            GlStateManager.translate(x + 12 + (width-2) * scale,y - 16 - (height+2) * scale,0);
            GL11.glScalef(scale, scale, scale);

            var color = new Color(((BlockShulkerBox) ((ItemShulkerBox) stack.getItem()).getBlock()).getColor().getColorValue());
            ...
            List<ItemStack> list = shulkerBoxItems(stack);
            var w = 0;
            var h = 0;
            ...
            for (final ItemStack itemStack : list) {
                if (!itemStack.isEmpty()) {
                    ...
                    mc.getRenderItem().renderItemAndEffectIntoGUI(itemStack, w * 18 - width + 1, h * 18 - height + 1);
                    mc.getRenderItem().renderItemOverlays(mc.fontRenderer, itemStack, w * 18 - width + 1, h * 18 - height + 1);
    ...
    {
        if (!(component == null || component.getStyle().getClickEvent() == null || !KevinClient.moduleManager.getModule("ComponentOnHover").getState())) {
