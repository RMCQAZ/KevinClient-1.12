...
public class PlayerControllerMP
{
    ...
    public boolean onPlayerDestroyBlock(BlockPos pos)
    {
        ...
        ItemStack heldItem = this.mc.player.getHeldItemMainhand();
        AutoTool autoTool = (AutoTool) KevinClient.moduleManager.getModule("AutoTool");
        if (autoTool.getState()&&autoTool.getSilentValue().get()&&autoTool.getNowSlot()!=mc.player.inventory.currentItem){
            heldItem = mc.player.inventory.getStackInSlot(autoTool.getNowSlot());
        }

        if (this.currentGameType.isCreative() && /*this.mc.player.getHeldItemMainhand()*/!heldItem.isEmpty() && /*this.mc.player.getHeldItemMainhand()*/heldItem.getItem() instanceof ItemSword)
        {
            return false;
        }
        else
        {
            ...
            else
            {
                ...
                if (!this.currentGameType.isCreative())
                {
                    ...
                    if (autoTool.getState()&&autoTool.getSilentValue().get()&&autoTool.getNowSlot()!=mc.player.inventory.currentItem) {
                        itemstack1 = mc.player.inventory.getStackInSlot(autoTool.getNowSlot());
                    }
    ...
    {
        final ClickWindowEvent event = new ClickWindowEvent(windowId, slotId, mouseButton, type);
