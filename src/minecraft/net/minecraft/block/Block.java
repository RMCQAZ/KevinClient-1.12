...
public class Block
{
    ...
    protected static void addCollisionBoxToList(BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable AxisAlignedBB blockBox)
    {
        if (blockBox != NULL_AABB)
        {
            ...
            WorldClient world = Minecraft.getMinecraft().world;
            if (world != null) {
                BlockBBEvent blockBBEvent = new BlockBBEvent(pos, world.getBlockState(pos).getBlock(), null);
                EventManager.INSTANCE.callEvent(blockBBEvent);
                if (blockBBEvent.getBoundingBox() != null && entityBox.intersects(blockBBEvent.getBoundingBox()))
                {
                    collidingBoxes.add(blockBBEvent.getBoundingBox());
                }
            }
        }
    }
    ...
    public float getPlayerRelativeBlockHardness(IBlockState state, EntityPlayer player, World worldIn, BlockPos pos)
    {
        ...
        if (f < 0.0F)
            ...
            ret = !player.canHarvestBlock(state) ? player.getDigSpeed(state) / f / 100.0F : player.getDigSpeed(state) / f / 30.0F;
        ...
        if (Objects.requireNonNull(noSlowBreak).getState()) {
            if (noSlowBreak.getWaterValue().get() && player.isInsideOfMaterial(Material.WATER) &&
                    !EnchantmentHelper.getAquaAffinityModifier(player)) {
                ret *= 5.0F;
            }

            if (noSlowBreak.getAirValue().get() && !player.onGround) {
                ret *= 5.0F;
            }
        } else if (player.onGround) { // NoGround
