...
object AltManager : GuiScreen() {
    ...
    private val file = FileManager.altsFile
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        RainbowFontShader.begin(true,-0.00314514F,0.00314514F,System.currentTimeMillis() % 10000 / 10000F).use {
            FontManager.font40.drawString("AltManager",this.width/2F-FontManager.font40.getStringWidth("AltManager")/2F,5F,0)
        }
        var stateX = this.width/2F-FontManager.font35.getStringWidth(stateMessage)/2F
        if (stateMessage.startsWith("§")) stateX += FontManager.font35.getStringWidth("§c")/2F
        FontManager.font35.drawString(stateMessage,stateX,10F+FontManager.font40.fontHeight,Color.lightGray.rgb)
        var yO = 1F - startY
        var c = 0
        ...
        removeButton.x = mc.currentScreen!!.width - 75
        changeButton.x = mc.currentScreen!!.width - 75
        loginButton.x = mc.currentScreen!!.width - 75
    ...
    data class Alt(var name:String,var password:String,var inGameName: String){
        ...
        fun getHeight() = 1 + FontManager.font35.fontHeight*.8F + FontManager.font35.fontHeight*.6F + 2F

        fun drawAlt(x: Float,x2: Float,y: Float,chose: Boolean): Float{
            var y2 = y + 1
            glPushMatrix()
            glScaled(.8,.8,.8)
            val fx1 = x + (x2-x)/2F - FontManager.font35.getStringWidth(inGameName)*.8F/2F
            FontManager.font35.drawString(inGameName,fx1/.8F,y2/.8F,if (chose) Color(0,111,255).rgb else Color.white.rgb)
            glPopMatrix()
            y2 += FontManager.font35.fontHeight*.8F
            glPushMatrix()
            glScaled(.6,.6,.6)
            val nameAndPassword = if (password.isEmpty()) name else "$name:******${password.removeRange(0..2)}"
            val fx2 = x + (x2-x)/2F - FontManager.font35.getStringWidth(nameAndPassword)*.6F/2F
            FontManager.font35.drawString(nameAndPassword,fx2/.6F,y2/.6F,Color.lightGray.rgb)
            glPopMatrix()
            y2 += FontManager.font35.fontHeight*.6F
            if (chose){
    ...
    class GuiAddAlt(private val name: String? = null,private val password: String? = null,private val title: String) : GuiScreen(){
        private val nameText:GuiTextField by lazy {
            GuiTextField(1,mc.fontRenderer,this.width/2-75,60,150,20)
        }
        private val passwordText:GuiTextField by lazy {
            GuiTextField(2,mc.fontRenderer,this.width/2-75,85,150,20)
        }
        private val nameAndPasswordText:GuiTextField by lazy {
            GuiTextField(3,mc.fontRenderer,this.width/2-75,125,150,20)
        }
        ...
        override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
            ...
            RainbowFontShader.begin(true,-0.001F,-0.001F,System.currentTimeMillis() % 10000 / 10000F).use {
                FontManager.font40.drawString(title,this.width/2-FontManager.font40.getStringWidth(title)/2F,8F,0)
            }
            nameText.drawTextBox()
            passwordText.drawTextBox()
            nameAndPasswordText.drawTextBox()
            val nameAndPassword = "Use Name And Password"
            FontManager.font35.drawString(nameAndPassword,this.width/2-FontManager.font35.getStringWidth(nameAndPassword)/2F,60F-FontManager.font35.fontHeight-4F,Color(0,255,0).rgb)
            val namePassword = "Use Name:Password"
            FontManager.font35.drawString(namePassword,this.width/2-FontManager.font35.getStringWidth(namePassword)/2F,125F-FontManager.font35.fontHeight-4F,Color(0,255,0).rgb)
            val nameStr = "Name:"
            val passwordStr = "Password:"
            FontManager.font40.drawString(nameStr,this.width/2-FontManager.font40.getStringWidth(nameStr)-80F,60F+(10-FontManager.font40.fontHeight/2),Color(0,111,255).rgb)
            FontManager.font40.drawString(passwordStr,this.width/2-FontManager.font40.getStringWidth(passwordStr)-80F,85F+(10-FontManager.font40.fontHeight/2),Color(0,111,255).rgb)
            nameText.x = this.width/2-75
            passwordText.x = this.width/2-75
            nameAndPasswordText.x = this.width/2-75
            buttonDone.x = this.width/2-75
            buttonDone.y = this.height/4*3-25
            var messageX = this.width/2-FontManager.font35.getStringWidth(message)/2F
            if (message.startsWith("§")) messageX += +FontManager.font35.getStringWidth("§c")/2F
            FontManager.font35.drawString(message,messageX,this.height/4F*3F+21F+FontManager.font35.fontHeight,Color.lightGray.rgb)
