package kevin.utils.rainbow

import kevin.utils.Mc
import org.apache.commons.io.IOUtils
import org.lwjgl.opengl.*

abstract class Shader(fragmentShader: String) : Mc() {
    var programId: Int = 0
    ...
    init {
        ...
        var con = true
        try {
            val vertexStream = javaClass.getResourceAsStream("/kevin/utils/gl/vertex.vert")
            vertexShaderID = createShader(IOUtils.toString(vertexStream), ARBVertexShader.GL_VERTEX_SHADER_ARB)
            IOUtils.closeQuietly(vertexStream)
            val fragmentStream =
                javaClass.getResourceAsStream("/kevin/utils/gl/$fragmentShader")
            fragmentShaderID =
