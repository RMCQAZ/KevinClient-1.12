...
class EditorPanel(private val hudDesigner: GuiHudDesigner, var x: Int, var y: Int) : Mc() {
    ...
    private fun drawCreate(mouseX: Int, mouseY: Int) {
        ...
        for (element in elements) {
            ...
            FontManager.font35.drawString(name, x + 2.0f, y + height.toFloat(), Color.WHITE.rgb)

            val stringWidth = FontManager.font35.getStringWidth(name)
        ...
        FontManager.font35.drawString("§lCreate element", x + 2F, y + 3.5F, Color.WHITE.rgb)
    }
    ...
    private fun drawSelection(mouseX: Int, mouseY: Int) {
        ...
        FontManager.font35.drawString("§lCreate element", x + 2.0f, y.toFloat() + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height
            && mouseY <= y + height + 10)
            create = true
        ...
        FontManager.font35.drawString("§lReset", x + 2.toFloat(), y.toFloat() + height, Color.WHITE.rgb)
        if (Mouse.isButtonDown(0) && !mouseDown && mouseX >= x && mouseX <= x + width && mouseY >= y + height
            && mouseY <= y + height + 10)
        ...
        FontManager.font35.drawString("§lAvailable Elements", x + 2.0f, y + height.toFloat(), Color.WHITE.rgb)
        height += 10
        realHeight += 10

        for (element in KevinClient.hud.elements) {
            FontManager.font35.drawString(element.name, x + 2f, y + height.toFloat(), Color.WHITE.rgb)

            val stringWidth = FontManager.font35.getStringWidth(element.name)
            if (width < stringWidth + 8)
                width = stringWidth + 8
        ...
        FontManager.font35.drawString("§lEditor", x + 2F, y + 3.5f, Color.WHITE.rgb)
    }
    ...
    private fun drawEditor(mouseX: Int, mouseY: Int) {
        ...
        // X
        FontManager.font35.drawString("X: ${"%.2f".format(element.renderX)} (${"%.2f".format(element.x)})", x + 2f, y + height.toFloat(), Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Y
        FontManager.font35.drawString("Y: ${"%.2f".format(element.renderY)} (${"%.2f".format(element.y)})", x + 2f, y + height.toFloat(), Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Scale
        FontManager.font35.drawString("Scale: ${"%.2f".format(element.scale)}", x + 2f, y + height.toFloat(), Color.WHITE.rgb)
        height += 10
        realHeight += 10

        // Horizontal
        FontManager.font35.drawString("H:", x + 2f, y + height.toFloat(), Color.WHITE.rgb)
        FontManager.font35.drawString(element.side.horizontal.sideName,
        ...
        // Vertical
        FontManager.font35.drawString("V:", x + 2f, y + height.toFloat(), Color.WHITE.rgb)
        FontManager.font35.drawString(element.side.vertical.sideName,
            x + 12F, y + height.toFloat(), Color.GRAY.rgb)
        ...
        for (value in element.values) {
            when (value) {
                is BooleanValue -> {
                    // Title
                    FontManager.font35.drawString(value.name, x + 2f, y + height.toFloat(), if (value.get()) Color.WHITE.rgb else Color.GRAY.rgb)

                    val stringWidth = FontManager.font35.getStringWidth(value.name)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8
                ...
                is FloatValue -> {
                    ...
                    FontManager.font35.drawString(text, x + 2f, y + height.toFloat(), Color.WHITE.rgb)

                    val stringWidth = FontManager.font35.getStringWidth(text)
                    if (width < stringWidth + 8)
                        width = stringWidth + 8
                ...
                is ListValue -> {
                    // Title
                    FontManager.font35.drawString(value.name, x + 2f, y + height.toFloat(), Color.WHITE.rgb)
                    ...
                    for (s in value.values) {
                        // Value title
                        val text = "§c> §r$s"
                        FontManager.font35.drawString(text, x + 2f, y + height.toFloat(), if (s == value.get()) Color.WHITE.rgb else Color.GRAY.rgb)
        ...
        // Header
        RenderUtils.drawRect(x, y, x + width, y + 12, Color(0,111,255).rgb)
        FontManager.font35.drawString("§l${element.name}", x + 2F, y + 3.5F, Color.WHITE.rgb)

        // Delete button
        if (!element.info.force) {
            val deleteWidth = x + width - FontManager.font35.getStringWidth("§lDelete") - 2F
            FontManager.font35.drawString("§lDelete", deleteWidth, y + 3.5F, Color.WHITE.rgb)
