...
class PacketCounter(x: Double = 100.0, y: Double = 30.0, scale: Float = 1F, side: Side = Side(Side.Horizontal.LEFT, Side.Vertical.UP)) : Element(x, y, scale, side),Listenable {
    ...
    override fun drawElement(): Border {
        ...
        for (i in receivedStart until receivedSize - 1) {
            ...
            GL11.glVertex2d(i.toDouble() - receivedStart, height.get()*2 + 1 - y.coerceAtMost(height.get().toFloat()).toDouble() + if (messageMode.equals("Up",true)) FontManager.font35.fontHeight else 0)
            GL11.glVertex2d(i + 1.0 - receivedStart, height.get()*2 + 1 - y1.coerceAtMost(height.get().toFloat()).toDouble() + if (messageMode.equals("Up",true)) FontManager.font35.fontHeight else 0)
        }
        ...
        if (!messageMode.equals("None",true)) {
            ...
            if (messageMode.equals("Right",true)) {
                ...
                FontManager.font35.drawString(
                    "Sent ${sentPacketsList.last()} packets in the past $delay MS.",
                    0F,
                    (-FontManager.font35.fontHeight/2) / 0.6F,
                    Color(255, 0, 0, 255).rgb
                )
                FontManager.font35.drawString(
                    "Received ${receivedPacketsList.last()} packets in the past $delay MS.",
                    0F,
                    (height.get() + FontManager.font35.fontHeight/2) / 0.6F,
                    Color(0, 255, 0, 255).rgb
                )
            }
            GL11.glPopMatrix()
        }
        val x2 = if (!messageMode.equals("Up",true)) FontManager.font35.getStringWidth("Received ${receivedPacketsList.last()} packets in the past $delay MS.").toFloat() else (receivedPacketsList.size - receivedStart).toFloat()
        val y2 = height.get()*2 + 2F + if (!messageMode.equals("Up",true)) FontManager.font35.fontHeight else 0
