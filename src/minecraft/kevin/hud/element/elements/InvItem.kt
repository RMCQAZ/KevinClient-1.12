...
class InvItem(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F) : Element(x, y, scale){
    ...
    private val startY = -(6+FontManager.font35.fontHeight).toFloat()
    ...
    private fun drawItems(slot: Int,endSlot: Int,y: Int){
        ...
        for(i in slot..endSlot){
            ...
            mc.renderItem.renderItemOverlays(mc.fontRenderer, stack, xOffset-18, y)
        }
    }
        ...
        if (titleRainbow.get()) {
            RainbowFontShader.begin(true, 1.0F / 1000, 1.0F / 1000, System.currentTimeMillis() % 10000 / 10000F).use {
                FontManager.font35.drawString(
                    name,
                    (174F / 2F) - (FontManager.font35.getStringWidth(name) * 0.5F),
                    -(FontManager.font35.fontHeight).toFloat(),
                    0,
                    false
                )
            }
    ...
    private fun invEmpty():Boolean{
        for(slot in 9..35){
            if(mc.player.inventoryContainer?.getSlot(slot)?.stack != null) return false
        }
        return true
    }
        ...
        if (invEmptyStringRainbow.get()) {
            RainbowFontShader.begin(true, 1.0F / 1000, 1.0F / 1000, System.currentTimeMillis() % 10000 / 10000F).use {
                FontManager.font35.drawString(
                    ...
                    (174F / 2F) - (FontManager.font35.getStringWidth("Your inventory is empty...") * 0.5F),
                    -(FontManager.font35.fontHeight) + 30.toFloat(),
                    0,
                    false
                )
            }
