...
class Information(x: Double = 0.0, y: Double = 30.0, scale: Float = 1F,side: Side = Side(Side.Horizontal.MIDDLE, Side.Vertical.UP)) : Element(x, y, scale, side),Listenable {
    ...
    override fun drawElement(): Border {
        ...
        FontManager.font35.drawString("Information",(10+45-FontManager.font35.getStringWidth("Information")/2*0.7f)/0.7f,(10+FontManager.font35.fontHeight/2F)/0.7f,Color(0,60,255).rgb)
        GL11.glPopMatrix()
        var y = 10.0 + FontManager.font35.fontHeight
        ...
        FontManager.font40.drawString("TPS: $tps",20/0.6f,(y.toFloat()+2+FontManager.font40.fontHeight/2F)/0.6f,if(tps >= 19.0) Color(0,255,0,200).rgb else Color(255,0,0,200).rgb)
        y += 2+FontManager.font40.fontHeight/2F
        FontManager.font40.drawString("HurtTime: ${mc.player.hurtTime}",20/0.6f,(y.toFloat()+2+FontManager.font40.fontHeight/2F)/0.6f,if (mc.player.hurtTime>0) Color(255,0,0,200).rgb else Color(0,20,255,200).rgb)
        y += 2+FontManager.font40.fontHeight/2F
        FontManager.font40.drawString("Kills: $kills",20/0.6f,(y.toFloat()+2+FontManager.font40.fontHeight/2F)/0.6f,Color(255,0,0,200).rgb)
        y += 2+FontManager.font40.fontHeight/2F
        FontManager.font40.drawString("Speed: ${bps}BPS",20/0.6f,(y.toFloat()+2+FontManager.font40.fontHeight/2F)/0.6f,Color(0,40,255,200).rgb)
        y += 2+FontManager.font40.fontHeight/2F
        FontManager.font40.drawString("Timeplayed: ${getTime(System.currentTimeMillis()-startTime)}",(10+45-FontManager.font40.getStringWidth("Timeplayed: ${getTime(System.currentTimeMillis()-startTime)}")/2*0.6f)/0.6f,(y.toFloat()+2+FontManager.font40.fontHeight/2F)/0.6f,Color(0,0,0,200).rgb)
        GL11.glPopMatrix()
    ...
    fun onPacket(event: PacketEvent){
        ...
        if (packet !is SPacketChat && packet.javaClass.name.contains("net.minecraft.network.play.server.",true)) timeTimer.reset()
    }
    @EventTarget
    ...
    private fun updateBPS(){
        if (mc.player == null || mc.player.ticksExisted < 1) bps=0.0
