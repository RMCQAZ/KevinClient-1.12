...
object ImageManager {
    ...
    private val jsonFile = File(FileManager.serverIconsDir,"config.json")
    fun load(){
    ...
    fun saveIcon(icon: BufferedImage,serverIP: String){
        try {
            val files = FileManager.serverIconsDir.listFiles()
            val ip = if (serverIP.contains(":")) serverIP.replace(":","_") else serverIP
            val name = "${files?.size} IP-$ip "
            val file = File(FileManager.serverIconsDir,"$name.png")
            var save = true
