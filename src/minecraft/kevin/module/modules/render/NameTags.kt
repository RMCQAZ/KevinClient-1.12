...
class NameTags : Module("NameTags", "Changes the scale of the nametags so you can always read them.", category = ModuleCategory.RENDER) {
    ...
    private val fontValue = FontManager.font40
    ...
    private fun renderNameTag(entity: EntityLivingBase, tag: String) {
        ...
        // Scale
        var distance = mc.player.getDistanceToEntity(entity) / 4F
        ...
        when(modeValue.get().toLowerCase()) {
            ...
            "liquid" -> {
                ...
                val distanceText = if (distanceValue.get()) "§7 [§a${mc.player.getDistanceToEntity(entity).roundToInt()}§7]" else ""
                ...
                if (armorValue.get() && entity is EntityPlayer) {
                    for (index in 0..5) {
                        if (entity.getItemStackFromSlot(index.toEntityEquipmentSlot()).isEmpty) continue

                        mc.renderItem.zLevel = -147F
                        mc.renderItem.renderItemAndEffectIntoGUI(entity.getItemStackFromSlot(index.toEntityEquipmentSlot()), -50 + index * 20, -22)
                    }
    ...
    private fun Int.toEntityEquipmentSlot(): EntityEquipmentSlot {
        return when (this) {
