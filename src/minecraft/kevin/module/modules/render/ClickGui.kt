...
import kevin.utils.rainbow.RainbowShader
...
class ClickGui : Module("ClickGui","Opens the ClickGUI.", category = ModuleCategory.RENDER, keyBind = Keyboard.KEY_RSHIFT) {
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is SPacketCloseWindow && (mc.currentScreen is ClickGUI || mc.currentScreen is NewClickGui)) {
            event.cancelEvent()
        }
    }
    ...
    class ClickGUI : GuiScreen(){
        ...
        override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
            ...
            FontManager.font180.drawStringWithShadow("K",mc.currentScreen!!.width/4F + 3,mc.currentScreen!!.height/4F - 4,Color(0,114,255).rgb)
            ...
            glVertex2f(mc.currentScreen!!.width/4F, mc.currentScreen!!.height/4F - 4 + FontManager.font180.fontHeight)
            ...
            glVertex2f(mc.currentScreen!!.width/8F*3, mc.currentScreen!!.height/4F - 4 + FontManager.font180.fontHeight)
            ...
            var y = mc.currentScreen!!.height/4F - 4 + FontManager.font180.fontHeight
            val high = ((mc.currentScreen!!.height/4F*3) - (mc.currentScreen!!.height/4F - 4 + FontManager.font180.fontHeight)) / 8
            for (c in category){
                ...
                RainbowFontShader.begin(fontRainbow,-0.001F,-0.001F,System.currentTimeMillis() % 10000 / 10000F).use {
                    FontManager.font35.drawString(c,mc.currentScreen!!.width/4F+(mc.currentScreen!!.width/8F*3-mc.currentScreen!!.width/4F)/2 - FontManager.font35.getStringWidth(c)/2,y+high/2-FontManager.font35.fontHeight/3,Color(0,111,255).rgb)
                }
            ...
            when(categoryOpen){
                ...
                8 -> {
                    ...
                    for (s in (KevinClient.moduleManager.getModule("Targets") as Targets).values){
                        ...
                        FontManager.font40.drawString(s.name,(moduleX + 3)/0.8F,(setY + 3)/0.8F,Color(0,0,0).rgb)
                        glPopMatrix()
                        ...
                        setY += FontManager.font40.fontHeight + 2
                    }
                }
            }
        ...
        override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
            ...
            val high = ((mc.currentScreen!!.height/4F*3) - (mc.currentScreen!!.height/4F - 4 + FontManager.font180.fontHeight)) / 8
            for (c in category){
                if (isClick(mc.currentScreen!!.width/4F,y,mc.currentScreen!!.width/8F*3,y+high,mouseX.toFloat(),mouseY.toFloat())) {if (mouseButton == 0) {categoryOpen = category.indexOf(c)+1;mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1f))} else {if (categoryOpen == category.indexOf(c)+1) {categoryOpen = 0;mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 0.6114514191981f));break} else {categoryOpen = category.indexOf(c)+1;mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1f))}}}
                y += high
            }
            val moduleY = mc.currentScreen!!.height/4F
            when(categoryOpen){
        ...
        private fun drawModule(category: ModuleCategory,start: Int,mouseX: Int,mouseY: Int){
            ...
            for (m in KevinClient.moduleManager.modules){
                ...
                RainbowFontShader.begin(fontRainbow,-0.001F,-0.001F,System.currentTimeMillis() % 10000 / 10000F).use {
                    FontManager.font35.drawString(
                        ...
                        y - FontManager.font35.fontHeight / 3 + moduleHigh / 2,
                        Color(0, 111, 255).rgb
                    )
                }
        ...
        private fun setModuleOpen(y: Float,moduleHigh: Float,moduleCategory: String,mouseButton: Int){
            if (mouseButton == 0) {moduleOpen!![moduleCategory] = ((y - mc.currentScreen!!.height / 4F) / moduleHigh).toInt();mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1f));settingStart!![moduleCategory] = 0} else {
                if (moduleOpen!![moduleCategory] == ((y - mc.currentScreen!!.height / 4F) / moduleHigh).toInt()) {moduleOpen!![moduleCategory] = -114514;mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 0.6114514191981f));settingStart!![moduleCategory] = 0} else {
        ...
        private fun drawSettings(module: Module,start: Int,mouseX: Int,mouseY: Int){
            ...
            for (v in values){
                when(v){
                    is BooleanValue ->{
                        if (y + FontManager.font35.fontHeight >mc.currentScreen!!.height/4F*3) {canSettingRoll2 = true;break}
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight;continue}
                        glPushMatrix()
                        glScaled(0.6,0.6,0.6)
                        FontManager.font35.drawString(v.name,(mc.currentScreen!!.width / 16F * 9 + 5)/0.6F,y/0.6F,Color(0,0,0).rgb)
                        glPopMatrix()
                        ...
                        y += FontManager.font35.fontHeight
                    }
                    is ListValue -> {
                        if (y + FontManager.font35.fontHeight * 0.6F > mc.currentScreen!!.height/4F*3) {canSettingRoll2 = true;break}
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 0.6F;continue}
                        var x = 0
                        val maxWidth = (mc.currentScreen!!.width / 4F * 3) - (mc.currentScreen!!.width / 16F * 9 + 5)
                        glPushMatrix()
                        glScaled(0.6,0.6,0.6)
                        FontManager.font35.drawString(v.name,(mc.currentScreen!!.width / 16F * 9 + 5)/0.6F,y/0.6F,Color(0,0,0).rgb)
                        y += FontManager.font35.fontHeight * 0.6F
                        for (i in v.values){
                            if (y + FontManager.font35.fontHeight * 0.6F > mc.currentScreen!!.height/4F*3) {canSettingRoll2 = true;break}
                            if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 0.6F;continue}
                            if (x + FontManager.font35.getStringWidth(i) > maxWidth) {y += FontManager.font35.fontHeight * 0.6F;x = 0}
                            val color = if (v.get() == i) Color(0,111,255).rgb else Color(150,150,150).rgb
                            FontManager.font35.drawString(i,(mc.currentScreen!!.width / 16F * 9 + 5 + x)/0.6F,y/0.6F,color)
                            x += FontManager.font35.getStringWidth(i)
                        }
                        y += FontManager.font35.fontHeight
                        glPopMatrix()
                    }
                    is TextValue -> {
                        if (y + FontManager.font35.fontHeight * 1.6F > mc.currentScreen!!.height/4F*3) {canSettingRoll2 = true;break}
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 1.6F;continue}
                        glPushMatrix()
                        glScaled(0.6,0.6,0.6)
                        FontManager.font35.drawString(v.name,(mc.currentScreen!!.width / 16F * 9 + 5)/0.6F,y/0.6F,Color(0,0,0).rgb)
                        y += FontManager.font35.fontHeight * 0.6F
                        FontManager.font35.drawString(v.get(),(mc.currentScreen!!.width / 16F * 9 + 5)/0.6F,y/0.6F,Color(0,111,255).rgb)
                        glPopMatrix()
                        y += FontManager.font35.fontHeight
                    }
                    is IntegerValue -> {
                        if (y + FontManager.font35.fontHeight * 0.8F + 8 > mc.currentScreen!!.height/4F*3) {canSettingRoll2 = true;break}
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 0.8F + 8;continue}
                        glPushMatrix()
                        glScaled(0.6,0.6,0.6)
                        FontManager.font35.drawString(v.name,(mc.currentScreen!!.width / 16F * 9 + 5)/0.6F,y/0.6F,Color(0,0,0).rgb)
                        FontManager.font35.drawString(v.get().toString(),(mc.currentScreen!!.width / 4F * 3 - FontManager.font35.getStringWidth(v.get().toString())/2 - 10)/0.6F,y/0.6F,Color(0,0,0).rgb)
                        y += FontManager.font35.fontHeight*0.8F
                        glPopMatrix()
                        ...
                        if (isClick(mc.currentScreen!!.width / 16F * 9 + 5,y - 3,mc.currentScreen!!.width / 4F * 3 - 5,y + 3,mouseX.toFloat(),mouseY.toFloat())){
                            if (Mouse.isButtonDown(0)){
                                val i = MathHelper.clamp((mouseX - (mc.currentScreen!!.width / 16F * 9 + 5)) / ((mc.currentScreen!!.width / 4F * 3 - 5) - (mc.currentScreen!!.width / 16F * 9 + 5)).toDouble(),0.0,1.0)
                    ...
                    is BlockValue -> {
                        ...
                        glScaled(0.6,0.6,0.6)
                        FontManager.font35.drawString(v.name,(mc.currentScreen!!.width / 16F * 9 + 5)/0.6F,y/0.6F,Color(0,0,0).rgb)
                        FontManager.font35.drawString(BlockUtils.getBlockName(v.get()),(mc.currentScreen!!.width / 4F * 3 - 5 - FontManager.font35.getStringWidth(BlockUtils.getBlockName(v.get())))/0.6F,y/0.6F,Color(60,60,60).rgb)
                        glPopMatrix()
                        y += FontManager.font35.fontHeight
                    }
                }
        ...
        private fun settingsClick(module: Module,start: Int,mouseX: Int,mouseY: Int){
            ...
            for (v in values){
                when(v){
                    is BooleanValue ->{
                        if (y + FontManager.font35.fontHeight >mc.currentScreen!!.height/4F*3)break
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight;continue}
                        if (isClick(mc.currentScreen!!.width / 4F * 3 - 13,y,mc.currentScreen!!.width / 4F * 3 - 4,y + 5,mouseX.toFloat(),mouseY.toFloat())) {v.set(!v.get());if(v.get())mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1f))else mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 0.6114514191981f))}
                        y += FontManager.font35.fontHeight
                    }

                    is ListValue -> {
                        if (y + FontManager.font35.fontHeight * 0.6F > mc.currentScreen!!.height/4F*3)break
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 0.6F;continue}
                        var x = 0
                        val maxWidth = (mc.currentScreen!!.width / 4F * 3) - (mc.currentScreen!!.width / 16F * 9 + 5)
                        y += FontManager.font35.fontHeight * 0.6F
                        for (i in v.values){
                            ...
                            if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 0.6F;continue}
                            if (x + FontManager.font35.getStringWidth(i) > maxWidth) {y += FontManager.font35.fontHeight * 0.6F;x = 0}
                            val click = isClick(mc.currentScreen!!.width/16F*9+5+x,y,mc.currentScreen!!.width/16F*9+5+x+FontManager.font35.getStringWidth(i),y+FontManager.font35.fontHeight*0.6F,mouseX.toFloat(),mouseY.toFloat())
                            if (click) {v.set(i);mc.soundHandler.playSound(PositionedSoundRecord.getMasterRecord(SoundEvents.UI_BUTTON_CLICK, 1f))}
                            x += FontManager.font35.getStringWidth(i)
                        }
                        y += FontManager.font35.fontHeight
                    }
                    is TextValue -> {
                        if (y + FontManager.font35.fontHeight * 1.6F > mc.currentScreen!!.height/4F*3)break
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 1.6F;continue}
                        y += FontManager.font35.fontHeight * 0.6F
                        y += FontManager.font35.fontHeight
                    }
                    is IntegerValue -> {
                        if (y + FontManager.font35.fontHeight * 0.8F + 8 > mc.currentScreen!!.height/4F*3)break
                        if (y < mc.currentScreen!!.height/4F + 5) {y += FontManager.font35.fontHeight * 0.8F + 8;continue}
                        y += FontManager.font35.fontHeight*0.8F
                        y += 8
                    }
    ...
    class NewClickGui : GuiScreen(){
        ...
        override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
            ...
            FontManager.font35.drawString("x",(x2-radius.toFloat()/16F*13.75F)/0.5F,(y1+radius.toFloat()/4F*1.9F)/0.5F,Color.white.rgb)
            glPopMatrix()
            //DrawLine
            ...
            RainbowFontShader.begin(true,-0.00314514F,0.00314514F,System.currentTimeMillis() % 10000 / 10000F).use {
                FontManager.font35.drawString("Kevin",x1+10F,y1+6.5F,0)
            }
            //DrawModeButton
            val modeButtonLight = isClick(mouseX.toFloat(),mouseY.toFloat(),x2-32F,y1+9.5F,x2-20F,y1+7.25F+FontManager.font35.fontHeight*0.6F)
            glPushMatrix()
            glScaled(.6,.6,.6)
            FontManager.font35.drawString("D",(x2-35F)/.6F,(y1+9.5F)/.6F,if(mode==1)Color(255,255,255).rgb else Color(20,20,20,225).rgb)
            FontManager.font35.drawString("L",(x2-19F)/.6F,(y1+9.5F)/.6F,if(mode==1)Color(255,255,255).rgb else Color(20,20,20,225).rgb)
            glPopMatrix()
            animButton("ModeButton",10F,mode==1)
            drawButton1(x2-32F,y1+9.5F,x2-20F,y1+7.25F+FontManager.font35.fontHeight*0.6F,if (mode==1) if(modeButtonLight) Color(170,170,170) else Color.white else if(modeButtonLight) Color.darkGray else Color.black,Color(0,111,255),buttonsAnim["ModeButton"]!!)
            //DrawCategoryButtons
        ...
        override fun mouseClicked(mouseX: Int, mouseY: Int, mouseButton: Int) {
            ...
            //ModeButton
            if (isClick(mouseX.toFloat(),mouseY.toFloat(),x2-32F,y1+9.5F,x2-20F,y1+7.25F+FontManager.font35.fontHeight*0.6F)) mode = if (mode==1) 2 else 1
            //CategoryButtons
            categoryButtons.forEach {
        ...
        private class Button(val name: String,var x1: Float,var y1: Float,var x2: Float,var y2: Float,var color: Color,var fontColor: Color,private val text: String,private val textPos: TextPos) {
            ...
            fun drawButton(){
                drawButtonNoText()
                val fontHigh = FontManager.font35.fontHeight
                val x = ((x2-x1)-FontManager.font35.getStringWidth(text)*0.8f)/2F
                glPushMatrix()
                glScaled(0.8,0.8,0.8)
                if (textPos==TextPos.Middle) FontManager.font35.drawString(text,(x1+x)/0.8f,(y1+(y2-y1)/2F-fontHigh/4F)/0.8f,fontColor.rgb)
                if (textPos==TextPos.Left) FontManager.font35.drawString(text,(x1+5)/0.8f,(y1+(y2-y1)/2F-fontHigh/4F)/0.8f,fontColor.rgb)
                glPopMatrix()
            }
            enum class TextPos{
                Left,Middle
        ...
        private fun drawModules(moduleCategory: ModuleCategory,mouseX: Int,mouseY: Int,mouseButton: Int,click: Boolean,yS: Double,startX: Double,endX: Double){
            ...
            var r = 0
            for (it in KevinClient.moduleManager.modules) {
        ...
        private fun drawGuiModuleSetting(module: Module,yad: Float,isClick: Boolean,mouseX: Int,mouseY: Int){
            ...
            if (isClick) {
                ...
                //Open
                yO -= 2F
                val tr = ((yO + startY.toFloat() + FontManager.font35.fontHeight * 0.8F) - (yO + startY.toFloat()))/2.0
                ...
                if (l) module.toggle()
                yO += FontManager.font35.fontHeight * 0.8F + 4F
                //Description
                ...
                var cou = 0
                t.forEach {
                    if (FontManager.font35.getStringWidth("$text $it") * 0.7 < endX - startX) {
                ...
                textList.forEach { _ ->
                    yO += FontManager.font35.fontHeight * 0.7F
                }
                yO += 3F
                //Settings
                    ...
                    when (it) {
                        is BooleanValue -> {
                            ...
                            val cl = isClick(
                                ...
                                startY.toFloat() + yO,
                                endX.toFloat() - 5F,
                                startY.toFloat() + yO + FontManager.font35.fontHeight * 0.65F
                            ) && (mouseX.toFloat() in startX - 2F..endX + 2F && mouseY.toFloat() in startY - 2F..endY + 2F)
                            if (cl) it.set(!it.get())
                            yO += FontManager.font35.fontHeight * 0.65F + 3.5F
                        }
                        is TextValue -> {
                            yO += FontManager.font35.fontHeight * 0.65F
                            val tl = arrayListOf<String>()
                            var tt = ""
                            val tl2 = it.get().toCharArray()
                            var co = 0
                            tl2.forEach { ch ->
                                if (FontManager.font35.getStringWidth("$tt$ch") * 0.5 < endX - startX) {
                                    tt += ch
                                } else {
                                    tl += tt
                                    tt = "$ch"
                            ...
                            tl.forEach { _ ->
                                yO += FontManager.font35.fontHeight * 0.5F
                            }
                            yO += 3F
                        }
                        is IntegerValue -> {
                        ...
                        is ListValue -> {
                            ...
                            val p1y =
                                (startY.toFloat() + yO + FontManager.font35.fontHeight * 0.65F / 2)
                            val p23x = (endX.toFloat() - 5F)
                            if (isClick(
                                    mouseX.toFloat(),
                                    mouseY.toFloat(),
                                    (p1x + (p23x - p1x) / 2.0F) - ((p23x - p1x) / 2.0F),
                                    p1y + (-FontManager.font35.fontHeight * 0.65F / 2 + 1F),
                                    (p1x + (p23x - p1x) / 2.0F) + (p23x - p1x) / 2.0F,
                                    p1y + (+FontManager.font35.fontHeight * 0.65F / 2 - 1F)
                                )
                            ...
                            if (open || listAnim["$moduleName${it.name}"] != 0F) {
                                var ly = 0F
                                it.values.forEach { _ ->
                                    ly += FontManager.font35.fontHeight * 0.6F
                                }
                                val animY = ly * listAnim["$moduleName${it.name}"]!! / 100F
                                val yOf = yO + animY
                                it.values.forEach vfe@{ v ->
                                    val alpha = when {
                                        yOf > yO + FontManager.font35.fontHeight * 0.6F -> 255
                                        yOf in yO..yO + FontManager.font35.fontHeight * 0.6F -> {
                                            val ya = yOf - yO
                                            ((ya / (FontManager.font35.fontHeight * 0.6F)) * 255).toInt()
                                        }
                                        else -> 0
                                    }
                                    ...
                                    if (isClick(
                                            mouseX.toFloat(),
                                            mouseY.toFloat(),
                                            endX.toFloat() - 10F - FontManager.font35.getStringWidth(
                                                v
                                            ) * 0.6F,
                                            startY.toFloat() + yO,
                                            endX.toFloat() - 10F,
                                            startY.toFloat() + yO + FontManager.font35.fontHeight * 0.6F
                                        )
                                    ) it.set(v)
                                    yO += FontManager.font35.fontHeight * 0.6F
                                }
                            }
                            yO += 3F
                        }
                ...
                settings.forEach {
                    when (it) {
                        ...
                        is IntegerValue -> {
                            ...
                            val ra =
                                (FontManager.font35.fontHeight * 0.65F - FontManager.font35.fontHeight * 0.55F) / 2F
                            FontManager.font35.drawString(
                                "${it.get()}",
                                (endX.toFloat() - 5F - FontManager.font35.getStringWidth("${it.get()}") * 0.55F) / 0.55F,
                        ...
                        is ListValue -> {
                            ...
                            //FontManager.font35.drawString("<",(endX.toFloat()-5F-FontManager.font35.getStringWidth("<")*0.65F)/0.65F,(startY.toFloat()+yO)/0.65F,if(open) Color(0,111,255).rgb else if(isLight())Color.black.rgb else Color.white.rgb)
                            val p1x = (endX.toFloat() - 5F - 5F) / .65
                            val p1y =
                                (startY.toFloat() + yO + FontManager.font35.fontHeight * 0.65F / 2) / .65
                            ...
                            val isMouseOn = isClick(
                                ...
                                p1y.toFloat() * .65F + (-FontManager.font35.fontHeight * 0.65F / 2 + 1F),
                            ...
                            glVertex3d(
                                (p23x - p1x) / 2.0,
                                (-FontManager.font35.fontHeight * 0.65F / 2 + 1F) / .65,
                                .0
                            )
                            glVertex3d(
                                (p23x - p1x) / 2.0,
                            ...
                            if (open || listAnim["$moduleName${it.name}"] != 0F) {
                                ...
                                it.values.forEach vfe@{ v ->
                                    ...
                                    FontManager.font35.drawString(
                                        v,
                                        (endX.toFloat() - 10F - FontManager.font35.getStringWidth(v) * 0.6F) / 0.6F,
        ...
        private fun drawSettings(category: ModuleCategory,isClick: Boolean,mouseX: Int,mouseY: Int){
            ...
            for (it in KevinClient.moduleManager.modules){
                ...
                if (isClick) {
                    ...
                    //State
                    //A1
                    val alpha1 = getAlpha(animLineY,yO+startY.toFloat(),yO+startY.toFloat()+FontManager.font35.fontHeight*0.8F)
                    if (alpha1==0) return
                    yO -= 2F
                    //A2
                    val alpha2 = getAlpha(animLineY,yO+startY.toFloat(),yO+startY.toFloat()+FontManager.font35.fontHeight*0.8F)
                    if (alpha2==0) return
                    if (isClick(mouseX.toFloat(),mouseY.toFloat(),endX.toFloat()-25F,yO+startY.toFloat(),endX.toFloat()-5F,yO+startY.toFloat()+FontManager.font35.fontHeight*0.8F)) it.toggle()
                    yO += FontManager.font35.fontHeight*0.8F + 4F
                    //Description
                    val textList = arrayListOf<String>()
                    var text = ""
                    ...
                    textList.forEach { _ ->
                        //A3
                        val alpha3 = getAlpha(animLineY,yO+startY.toFloat(),yO+startY.toFloat()+FontManager.font35.fontHeight*0.7F)
                        if (alpha3 == 0) return
                    ...
                    settings.forEach{
                        when(it){
                            is BooleanValue -> {
                                //A4
                                val alpha4 = getAlpha(animLineY,startY.toFloat()+yO,startY.toFloat()+yO+FontManager.font35.fontHeight*0.65F)
                                if (alpha4 == 0) return
                                yO -= 1.5F
                                //A5
                                val alpha5 = getAlpha(animLineY,startY.toFloat()+yO,startY.toFloat()+yO+FontManager.font35.fontHeight*0.65F)
                                if (alpha5 == 0) return
                                val cl = isClick(mouseX.toFloat(),mouseY.toFloat(),endX.toFloat()-18F,startY.toFloat()+yO,endX.toFloat()-5F,startY.toFloat()+yO+FontManager.font35.fontHeight*0.65F) && (mouseX.toFloat() in startX-2F..endX+2F && mouseY.toFloat() in startY-2F..endY+2F)
                                if (cl) it.set(!it.get())
                                yO += FontManager.font35.fontHeight*0.65F+3.5F
                            }
                            is TextValue -> {
                                ...
                                tl.forEach { _ ->
                                    //A5
                                    val alpha5 = getAlpha(animLineY,yO+startY.toFloat(),FontManager.font35.fontHeight*0.5F)
                                    if (alpha5 == 0) return
                                    yO += FontManager.font35.fontHeight*0.5F
                                }
                                yO += 3F
                            ...
                            is IntegerValue -> {
                                ...
                                if (alpha4==0) return
                                //A5
                                val ra = (FontManager.font35.fontHeight*0.65F-FontManager.font35.fontHeight*0.55F)/2F
                                val alpha5 = getAlpha(animLineY,startY.toFloat()+yO+ra,startY.toFloat()+yO+ra+FontManager.font35.fontHeight*0.55F)
                                if (alpha5==0) return
                                yO += FontManager.font35.fontHeight*0.65F
                                yO += 1.5F
                                //A6
                            ...
                            is ListValue -> {
                                ...
                                if (alpha4 == 0) return
                                val p1x = (endX.toFloat()-5F-5F)
                                val p1y = (startY.toFloat()+yO+FontManager.font35.fontHeight*0.65F/2)
                                val p23x = (endX.toFloat()-5F)
                                //A5
                                val alpha5 = getAlpha(animLineY,p1y.toFloat()*.65F+ (-FontManager.font35.fontHeight*0.65F/2+1F),p1y.toFloat()*.65F+(+FontManager.font35.fontHeight*0.65F/2-1F))
                                if (alpha5 == 0) return
                                if (isClick(mouseX.toFloat(),mouseY.toFloat(),(p1x+(p23x-p1x)/2.0F)-((p23x-p1x)/2.0F),p1y+(-FontManager.font35.fontHeight*0.65F/2+1F),(p1x+(p23x-p1x)/2.0F)+(p23x-p1x)/2.0F,p1y+(+FontManager.font35.fontHeight*0.65F/2-1F))) listOpen["$moduleName${it.name}"] = !listOpen["$moduleName${it.name}"]!!
                                val open = listOpen["$moduleName${it.name}"]!!
                                yO += FontManager.font35.fontHeight*0.65F
                                if (open||listAnim["$moduleName${it.name}"] != 0F) {
                                    var ly = 0F
                                    ...
                                    it.values.forEach vfe@ { v ->
                                        //A6
                                        val alpha6 = getAlpha(animLineY,startY.toFloat()+yO,startY.toFloat()+yO+FontManager.font35.fontHeight*0.6F)
                                        if (alpha6 == 0) return
                                        val alpha = when {
                                            yOf > yO+FontManager.font35.fontHeight*0.6F -> 255
                                            yOf in yO..yO+FontManager.font35.fontHeight*0.6F -> {
                                        ...
                                        if (alpha == 0) return@vfe
                                        if (isClick(mouseX.toFloat(),mouseY.toFloat(),endX.toFloat()-10F-FontManager.font35.getStringWidth(v)*0.6F,startY.toFloat()+yO,endX.toFloat()-10F,startY.toFloat()+yO+FontManager.font35.fontHeight*0.6F)) it.set(v)
                                        yO += FontManager.font35.fontHeight*0.6F
                                    }
                                }
                                yO += 3F
                    ...
                    FontManager.font35.drawString("State",(startX.toFloat()+1F)/0.8F,(yO+startY.toFloat())/0.8F,Color(0,111,255,alpha1).rgb)
                    ...
                    yO -= 2F
                    val l = isClick(mouseX.toFloat(),mouseY.toFloat(),endX.toFloat()-25F,yO+startY.toFloat(),endX.toFloat()-5F,yO+startY.toFloat()+FontManager.font35.fontHeight*0.8F) && (mouseX.toFloat() in startX-2F..endX+2F && mouseY.toFloat() in startY-2F..endY+2F)
                    ...
                    textList.forEach {
                        ...
                        FontManager.font35.drawString(it,(startX.toFloat()+1F)/0.7F,(yO+startY.toFloat())/0.7F,if(isLight())Color(0,0,0,alpha3).rgb else Color(255,255,255,alpha3).rgb)
                    ...
                    settings.forEach{
                        when(it){
                            is BooleanValue -> {
                                ...
                                FontManager.font35.drawString(it.name,(startX.toFloat()+1F)/0.65F,(startY.toFloat()+yO)/0.65F,if(isLight())Color(0,0,0,alpha4).rgb else Color(255,255,255,alpha4).rgb)
                            ...
                            is TextValue -> {
                                ...
                                tl.forEach { s ->
                                    ...
                                    FontManager.font35.drawString(s,(startX.toFloat()+1F)/0.5F,(yO+startY.toFloat())/0.5F,Color(0,111,255,alpha5).rgb)
                            ...
                            is IntegerValue -> {
                                ...
                                FontManager.font35.drawString("${it.get()}",(endX.toFloat()-5F-FontManager.font35.getStringWidth("${it.get()}")*0.55F)/0.55F,(startY.toFloat()+yO+ra)/0.55F,if(isLight())Color(0,0,0,alpha5).rgb else Color(255,255,255,alpha5).rgb)
                                ...
                                if (isClicked){
                                    val i = MathHelper.clamp((x - (startX+5.0)) / ((endX-5.0)-(startX+5.0)),0.0,1.0)
                                    it.set((it.minimum + (it.maximum - it.minimum) * i).toInt())
                                }
                                yO += 1.5F
                                yO += 4F
                            ...
                            is ListValue -> {
                                ...
                                //FontManager.font35.drawString("<",(endX.toFloat()-5F-FontManager.font35.getStringWidth("<")*0.65F)/0.65F,(startY.toFloat()+yO)/0.65F,if(open) Color(0,111,255).rgb else if(isLight())Color.black.rgb else Color.white.rgb)
                                val p1x = (endX.toFloat()-5F-5F)/.65
                                val p1y = (startY.toFloat()+yO+FontManager.font35.fontHeight*0.65F/2)/.65
                                val p23x = (endX.toFloat()-5F)/.65
                                //Alpha5
                                val alpha5 = getAlpha(animLineY,p1y.toFloat()*.65F+ (-FontManager.font35.fontHeight*0.65F/2+1F),p1y.toFloat()*.65F+(+FontManager.font35.fontHeight*0.65F/2-1F))
                                if (alpha5 == 0) {
                                ...
                                glDisable(GL_TEXTURE_2D)
                                glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
                                val isMouseOn = isClick(mouseX.toFloat(),mouseY.toFloat(),(p1x*.65F+(p23x*.65F-p1x*.65F)/2.0F).toFloat()-((p23x*.65F-p1x*.65F)/2.0F).toFloat(),p1y.toFloat()*.65F+ (-FontManager.font35.fontHeight*0.65F/2+1F),(p1x*.65F+(p23x*.65F-p1x*.65F)/2.0F).toFloat()+(p23x*.65F-p1x*.65F).toFloat()/2.0F,p1y.toFloat()*.65F+(+FontManager.font35.fontHeight*0.65F/2-1F))
                                ...
                                glBegin(GL_TRIANGLES)
                                glVertex3d(-((p23x-p1x)/2.0),.0,.0)
                                glVertex3d((p23x-p1x)/2.0,(-FontManager.font35.fontHeight*0.65F/2+1F)/.65,.0)
                                glVertex3d((p23x-p1x)/2.0,(+FontManager.font35.fontHeight*0.65F/2-1F)/.65,.0)
                                glEnd()
                                glEnable(GL_TEXTURE_2D)
                                glDisable(GL_BLEND)
                                ...
                                if (open||listAnim["$moduleName${it.name}"] != 0F) {
                                    ...
                                    it.values.forEach vfe@ { v ->
                                        ...
                                        val w = Color(255,255,255,alpha)
                                        val b = Color(0,0,0,alpha)
                                        FontManager.font35.drawString(v,(endX.toFloat()-10F-FontManager.font35.getStringWidth(v)*0.6F)/0.6F,(startY.toFloat()+yO)/0.6F,if(it.get()==v) Color(0,111,255,alpha).rgb else if(isLight()) b.rgb else w.rgb)
                                        yO += FontManager.font35.fontHeight*0.6F
                                    }
                                    glPopMatrix()
                                }
