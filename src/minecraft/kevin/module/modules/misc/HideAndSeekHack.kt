...
object HideAndSeekHack : Module("HideAndSeekHack","Mark every hider.") {
    ...
    fun isHider(entity: Entity?):Boolean{
        ...
        for (it in 36..44) {
            val itemStack = mc.player!!.inventoryContainer.getSlot(it).stack
            val item = itemStack.item
            if (!itemStack.isEmpty && item is ItemSword && item.material == Item.ToolMaterial.DIAMOND){
                return true
            }
        }
        return false
    ...
    fun onUpdate(event: UpdateEvent){
        ...
        for (x in -100..100){
            for (z in -100..100){
                ...
                if (mc.world.isAirBlock(blockPos)&&
                    !mc.world.isAirBlock(blockPos.add(0,-1,0))&&
                    mc.world.isBlockLoaded(blockPos)) {
                    airs.add(blockPos)
                }
            }
        }
        when(searchMode.get()){
            "Region" -> {
                ...
                if (sY>maxY.get()) {
                    ...
                    if (debug.get())
                        ChatUtils.messageWithPrefix("§l§7[§l§9HideAndSeekDebug§l§7] §l§9 XDeviation:$sX ZDeviation:$sZ HidePoints:${airs.size}")
                }
            }
            "Layer" -> {
                if (sX!=200){
                    ...
                    if (sZ!=200){
                        ...
                        if (debug.get())
                            ChatUtils.messageWithPrefix("§l§7[§l§9HideAndSeekDebug§l§7] §l§9 Y:$sY HidePoints:${airs.size}")
                    }
                }
            }
        }
        ...
        hiderBlocks.addAll(airs.filter {
            mc.world.isBlockLoaded(it)&&
            !mc.world.isAirBlock(it)&&
            mc.player.getDistance(it.x.toDouble(),it.y.toDouble(),it.z.toDouble())<40 })
        if (oldSize != hiderBlocks.size && debug.get()){ ChatUtils.messageWithPrefix("§l§7[§l§9HideAndSeekDebug§l§7] §l§9$oldSize -> ${hiderBlocks.size}") }
    }
    @EventTarget
