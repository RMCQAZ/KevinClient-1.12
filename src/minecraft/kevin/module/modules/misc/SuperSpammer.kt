...
class SuperSpammer : Module("SuperSpammer","Spams the chat with given messages.", category = ModuleCategory.MISC) {
    ...
    init {
        val files = FileManager.spammerDir
    ...
    private val messageValue = TextValue("SingleMessage", KevinClient.clientName + " Client | Jiege")
            ...
            if (mode.equals("Switch",true)) {
                ...
                when (mode.lowercase(Locale.getDefault())){
                    ...
                    "invincible" -> spammerList = invincibleList
                        ...
                        val files = FileManager.spammerDir.listFiles()!!
                        var file: File? = null
                        for (i in files){
                ...
                if (spammerList.isNotEmpty()) mc.player.sendChatMessage(if (customNoRandomV.get()) "$start${spammerList[sentencesNumber]}" else "$first${spammerList[sentencesNumber]}$last")
