...
class Step : Module("Step", "Allows you to step up/down blocks.", category = ModuleCategory.MOVEMENT) {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when {
            ...
            mode.equals("aac3.3.4", true) -> if (thePlayer.isCollidedHorizontally
                ...
                if (isAACStep) {
                    ...
                    if (!thePlayer.isHandActive && thePlayer.movementInput.moveStrafe == 0F)
        ...
        if (reverse.get()){
            ...
            if (collideBlock(thePlayer.entityBoundingBox, fun(block:Block?):Boolean {return block is BlockLiquid}) || collideBlock(
                    AxisAlignedBB(thePlayer.entityBoundingBox.maxX, thePlayer.entityBoundingBox.maxY, thePlayer.entityBoundingBox.maxZ, thePlayer.entityBoundingBox.minX, thePlayer.entityBoundingBox.minY - 0.01, thePlayer.entityBoundingBox.minZ), fun(block:Block?):Boolean {return block is BlockLiquid})) return
            ...
            when {
                mode.equals("NCP", ignoreCase = true) || mode.equals("AAC", ignoreCase = true) -> {
                    ...
                    mc.connection!!.sendPacket(
                        CPacketPlayer.Position(stepX,
                        stepY + 0.41999998688698, stepZ, false))
                    mc.connection!!.sendPacket(CPacketPlayer.Position(stepX,
    ...
    private fun fakeJump() {
        ...
        thePlayer.addStat(StatList.JUMP)
    }
    ...
    private fun couldStep(): Boolean {
        ...
        return mc.world!!.getCollisionBoxes(mc.player,mc.player!!.entityBoundingBox.offset(x, 1.001335979112147, z))
            .isEmpty()
    }

    fun isValidHeight(height: Double): Boolean {
        return height <= heightValue.get()
    }
