...
class WallClimb : Module("WallClimb", "Allows you to climb up walls like a spider.", category = ModuleCategory.MOVEMENT) {
    ...
    fun onBlockBB(event: BlockBBEvent) {
        ...
        when (mode.toLowerCase()) {
            ...
            "clip" -> if (event.block != null && mc.player != null && (event.block)is BlockAir && event.y < thePlayer.posY && thePlayer.isCollidedHorizontally && !thePlayer.isOnLadder && !thePlayer.isInWater && !thePlayer.isInLava) event.boundingBox = AxisAlignedBB(0.0, 0.0, 0.0, 1.0, 1.0, 1.0).offset(thePlayer.posX, thePlayer.posY.toInt() - 1.0, thePlayer.posZ)
        }
    }
}