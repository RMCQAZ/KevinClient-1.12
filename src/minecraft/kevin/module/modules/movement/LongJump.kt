...
class LongJump : Module("LongJump", "Allows you to jump further.", category = ModuleCategory.MOVEMENT) {
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if ((modeValue equal "Timer")&&msTimer.hasTimePassed(delay.get().toLong())){
            when(timerState){
                0 -> {
                    val radiansYaw = mc.player!!.rotationYaw * Math.PI / 180
                    mc.player!!.motionX = speed.get() * -sin(radiansYaw)
                    mc.player!!.motionZ = speed.get() * cos(radiansYaw)
        ...
        if (modeValue.get().equals("ExplosionBoost",true)){
            if (explosion){
                mc.player.motionX *= 1F + explosionBoostLong.get()
                mc.player.motionY *= 1F + explosionBoostHigh.get()
                mc.player.motionZ *= 1F + explosionBoostLong.get()
                explosion = false
            }
        }
    }
    ...
    fun onPacket(event: PacketEvent){
        if (event.packet is SPacketExplosion)
            if (event.packet.motionZ != 0F ||
                event.packet.motionY != 0F ||
                event.packet.motionX != 0F) explosion = true
    }
