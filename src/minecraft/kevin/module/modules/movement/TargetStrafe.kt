...
object TargetStrafe : Module("TargetStrafe","Strafe around your target.", category = ModuleCategory.MOVEMENT) {
    ...
    fun strafe(event: MoveEvent) {
        ...
        if (canStrafe(target))
            setSpeed(event, MovementUtils.speed.toDouble(), RotationUtils.getRotationsEntity(target).yaw, direction.toDouble(), if (mc.player.getDistanceToEntity(target) <= radius.get()) 0.0 else 1.0)
    }

    @EventTarget
    fun onMove(event: MoveEvent) {
        if (safewalk.get() && mc.player.onGround && canStrafe(KevinClient.combatManager.target))
    ...
    private fun canStrafe(target: EntityLivingBase?): Boolean {
        return this.state && target != null && (!space.get() || mc.player.movementInput.jump) && (!onlySpeed.get() || KevinClient.moduleManager.getModule("Speed")!!.state)
    }
}