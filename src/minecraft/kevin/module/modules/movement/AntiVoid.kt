...
class AntiVoid : Module("AntiVoid","Automatically setbacks you after falling a certain distance.", category = ModuleCategory.MOVEMENT) {
            ...
            if (thePlayer.fallDistance - lastFound > maxDistanceWithoutGround.get()) {
                ...
                when (mode.toLowerCase()) {
                    ...
                    "motionteleport-flag" -> {
                        ...
                        mc.connection!!.sendPacket(CPacketPlayer.Position(thePlayer.posX, thePlayer.posY, thePlayer.posZ, true))
                        thePlayer.motionY = 0.1
        ...
        if(modeValue.get().toLowerCase() == "minemora-blink" && !KevinClient.moduleManager.getModule("Fly")!!.state && !KevinClient.moduleManager.getModule("HighJump")!!.state){
            if(!blink){
                ...
                if(mc.player!!.fallDistance>5){
                    mc.player!!.setPositionAndUpdate(posX,posY+yBoost.get(),posZ)
            ...
            if(scaffoldOn){
                if(mc.player!!.onGround || mc.player!!.isOnLadder || mc.player!!.isInWater || mc.player!!.isInWeb || mc.player!!.fallDistance > 10){
    ...
    fun onPacket(event: PacketEvent){
        ...
        if(modeValue.get().toLowerCase() == "minemora-blink" && (packet)is CPacketPlayer && blink){
